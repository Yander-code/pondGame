...
void generatePondInf(){
	...
	allPond[14].inf = "2-6號夜鷺生態埤塘 地點：330桃園市桃園區國豐七街48號 簡介：古早時為竹子陂等3口小埤塘合併為一口大埤塘，又稱為「大埤」。在居民參與下，定名為「夜鷺（YaLe）生態埤塘」；工程包括了環湖步道800公尺，還有170公尺的鋼構步橋，直接由環湖步道通往龍安國小，安全又無車輛干擾。";
	...
	allPond[16].inf = "蘆竹六股生態埤塘 地點：桃園市蘆竹區大興一街、興仁路旁 簡介：六股埤塘生態公園舊名為2-11埤塘，總面積約4.5公頃，全長850公尺的全齡無障礙環埤塘步道，保留原有的植栽造景，並強化夜間照明設施，埤塘內有夜鷺、翠鳥等豐富的生態，也成為鄰近居民休憩運動的好去處，在埤塘旁的大竹圖書館讀書之餘，還能欣賞六股埤塘的夜景。";
...
int main(){
	...
	while(stop<100){
		...
		if(lastLoc!=0 && (lastLoc)%16*2!=(bLoc)%16*2){
			map[(lastLoc)%16*2]='O';
		...
		//埤塘or空地 特殊事件  
		std::cout << "你走到了: " << allPond[(aLoc)%17].inf << std::endl;
		if(allPond[(aLoc)%17].house==0 && allPond[(aLoc)%17].inf!="空地QQ"){
			...
			if(aMoney >= 1000){
				...
				if(tem=='Y'){
					aMoney-=1000;
					allPond[(aLoc)%17].house=1;
					std::cout<<"感謝您認養此埤塘"<<std::endl;
				} 
			}
		}else if(allPond[(aLoc)%17].house==2){
			std::cout << std::endl << allPond[(aLoc)%17].event << std::endl << std::endl;
			aMoney += allPond[(aLoc)%17].eventMoney;
			if((aLoc)%17==4){
				aLoc-=5;
		...
		if(lastLoc2!=0 && (lastLoc2)%16*2!=(aLoc)%16*2){
			map[(lastLoc2)%16*2]='O';
		...
		std::cout << "你走到了: " << allPond[(bLoc)%17].inf << std::endl;
		if(allPond[(bLoc)%17].house==0 && allPond[(bLoc)%17].inf!="空地QQ"){
			std::cout << std::endl << allPond[(aLoc)%17].event << std::endl << std::endl;
			aMoney += allPond[(aLoc)%17].eventMoney;
			if((aLoc)%17==4){
				aLoc-=5;
			...
			if(bMoney >= 1000){
				...
				if(tem=='Y'){
					bMoney-=1000;
					allPond[(bLoc)%17].house=2;
					std::cout<<"感謝您認養此埤塘"<<std::endl;
				} 
			}
		}else if(allPond[(bLoc)%17].house==1){
			std::cout << std::endl << allPond[(aLoc)%17].event << std::endl << std::endl;
			aMoney += allPond[(aLoc)%17].eventMoney;
			if((aLoc)%17==4){
				aLoc-=5;
